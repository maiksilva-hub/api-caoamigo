...
// MUDANÇA 1: PATH para Versão 2
@Path("/v2/cachorros")
@Consumes("application/json")
@Produces("application/json")
public class CachorroV2Resource { // MUDANÇA 2: Nome da classe

    // MUDANÇA 3: O GET ALL tem uma ordenação padrão diferente
    @GET
    @Operation(
            summary = "Retorna todos os cachorros (getAll) - V2",
            description = "V2: Retorna uma lista de cachorros ordenada pela data de nascimento (mais novo primeiro) por padrão."
    )
    @APIResponse(
            responseCode = "200",
    ...
    public Response getAll(){
        // V2 ALTERAÇÃO: Ordena por data de nascimento, decrescente (mais novo primeiro)
        return Response.ok(Cachorro.listAll(Sort.by("dataDeNascimento", Sort.Direction.Descending))).build();
    }
    ...
    @Path("{id}")
    @Operation(
            summary = "Retorna um cachorro pela busca por ID (getById) - V2",
    ...
    @GET
    @Operation(
            summary = "Retorna os cachorros conforme o sistema de pesquisa (search) - V2",
            description = "V2: Retorna uma lista de cachorros com ordenação padrão por data de nascimento."
    )
    @APIResponse(
            responseCode = "200",
            description = "Item retornado com sucesso",
    ...
    public Response search(
            ...
            @QueryParam("sort") @DefaultValue("dataDeNascimento") String sort, // MUDANÇA 4: Padrão V2
            @Parameter(description = "Esquema de filtragem de cachorros por ordem crescente ou decrescente")
            @QueryParam("direction") @DefaultValue("desc") String direction, // MUDANÇA 5: Padrão V2
        ...
        if(!allowed.contains(sort)){
            sort = "dataDeNascimento"; // MUDANÇA 6: Usar o novo padrão
        }
        ...
        // MUDANÇA 7: Atualiza NextPage para V2
        response.NextPage = response.HasMore ? "http://localhost:8080/v2/cachorros/search?q="+(q != null ? q : "")+"&page="+(effectivePage + 1) + (size > 0 ? "&size="+size : "") : "";
    ...
    @POST
    @Operation(
            summary = "Adiciona um registro à lista de cachorros (insert) - V2",
    ...
    public Response insert(@Valid Cachorro cachorro){
        ...
        // MUDANÇA 8: Ajusta URI de retorno para V2
        return Response.status(Response.Status.CREATED)
                .header("Location", "/v2/cachorros/" + cachorro.id)
    ...
    @DELETE
    @Operation(
            summary = "Remove um registro da lista de cachorros (delete) - V2",
    ...
    @PUT
    @Operation(
            summary = "Altera um registro da lista de cachorros (update) - V2",

...
public class AdocaoResource {
    ...
    public Response search(
        ...
        if (q == null || q.isBlank()) {
            ...
            try {
                // Tenta converter a pesquisa em data
                LocalDate data = LocalDate.parse(q);

                // Busca por data de solicitação
                query = Adocao.find(
                        "dataSolicitacao = ?1",
                        sortObj,
                        data
                ...
                // se não for data, busca só em campos textuais
                query = Adocao.find(
    ...
    public Response insert(@Valid Adocao adocao){
        ...
        // Resolver cachorro (pode ter apenas id)
        ...
        // Resolver raças (se vierem com id)
